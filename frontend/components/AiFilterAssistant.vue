  ...
  <div class="ai-assistant-card">
    <div class="card-header">
      <div class="header-title-flex">
        <Sparkles :size="18" class="text-emerald-700" />
        <h4>{{ t('ai_filter.title', 'Gerador de Filtros Inteligente') }}</h4>
      </div>
    ...
    <div class="input-row">
      ...
      <button @click="processNaturalLanguage" class="btn-generate" :disabled="isProcessing || !promptText.trim()">
        <span v-if="isProcessing" class="btn-flex-inner">
          <Loader2 class="spin-icon" :size="15" />
          {{ t('ai_filter.btn_processing', 'Processando...') }}
        </span>
        <span v-else class="btn-flex-inner">
          <Sparkles :size="15" />
          {{ t('ai_filter.btn_generate', 'Gerar Filtros') }}
        </span>
      </button>
    </div>
    ...
    <transition name="fade">
      <div v-if="aiResult" class="ai-result-box mt-3">
        <div class="result-header">
          ...
          <strong>{{ t('ai_filter.generated_title', 'Filtros Gerados:') }}</strong>
        </div>

        <div class="tags-group">
          <label class="label-with-icon">
            <Search :size="14" />
            {{ t('ai_filter.suggested_terms', 'Termos de Busca sugeridos:') }}
          </label>
          <div class="tags-list">
        ...
        <div class="tags-group mt-2">
          <label class="label-with-icon">
            <Ban :size="14" />
            {{ t('ai_filter.blacklist_label', 'Blacklist (Palavras a ignorar):') }}
          </label>
          <div class="tags-list">
        ...
        <button @click="applyAiFilters" class="btn-apply mt-3">
          <Check :size="16" />
          {{ t('ai_filter.btn_apply', 'Aplicar aos Filtros') }}
        </button>
      </div>
    </transition>
  </div>
...
<script setup>
import { ref } from 'vue'
import { Sparkles, Loader2, CheckCircle2, Search, Ban, Check } from 'lucide-vue-next'
...
.btn-flex-inner { display: flex; align-items: center; gap: 0.4rem; }
...
.result-header { color: #166534; font-size: 0.9rem; margin-bottom: 0.6rem; display: flex; align-items: center; gap: 0.4rem; }
.label-with-icon { display: flex; align-items: center; gap: 0.35rem; font-size: 0.8rem; font-weight: 700; color: #334155; margin-bottom: 0.3rem; }
...
.btn-apply { width: 100%; background: #2563eb; color: #ffffff; border: none; padding: 0.6rem; border-radius: 8px; font-weight: 700; cursor: pointer; transition: background 0.2s; font-size: 0.88rem; display: flex; align-items: center; justify-content: center; gap: 0.4rem; }
